...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .lp-category-section { margin-bottom: 28px; }
    .lp-search-wrap { position: relative; margin-bottom: 20px; }
    .lp-search-input { width: 100%; padding: 14px 18px 14px 46px; background: #fff; border: 1px solid var(--gold-border); border-radius: var(--radius-btn); font: inherit; color: var(--text-dark); outline: none; }
    .lp-search-input:focus { border-color: var(--accent-pink); box-shadow: 0 0 0 3px rgba(217, 101, 139, 0.12); }
    .lp-search-icon { position: absolute; left: 18px; top: 50%; transform: translateY(-50%); color: var(--accent-pink); font-size: 18px; }
    .lp-category-grid { display: flex; gap: 12px; overflow-x: auto; padding: 2px 2px 12px; scroll-snap-type: x mandatory; scrollbar-width: thin; }
    .lp-cat-card { flex: 0 0 190px; background: var(--card-bg); border: 1px solid var(--gold-border); border-radius: var(--radius-md); padding: 12px; text-align: left; cursor: pointer; transition: border-color 0.2s, transform 0.2s; display: flex; align-items: center; gap: 10px; scroll-snap-align: start; }
    .lp-cat-card:hover, .lp-cat-card.active { border-color: var(--accent-pink); transform: translateY(-2px); }
    .lp-cat-img { width: 52px; height: 52px; flex: 0 0 52px; object-fit: cover; border-radius: 12px; }
    .lp-cat-title { font-size: 13px; font-weight: 700; color: var(--text-dark); margin-bottom: 2px; }
    .lp-cat-desc { font-size: 10px; color: var(--text-muted); line-height: 1.3; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
    .lp-products-section { margin-bottom: 40px; }
    .lp-products-heading { font-family: 'Playfair Display', serif; font-size: 28px; margin-bottom: 16px; }
    ...
    .lp-inline-product-grid { margin-bottom: 0; }
    ...
    <section class="lp-products-section" id="products">
      ...
      <div class="lp-product-grid lp-inline-product-grid" id="inline-product-grid">
        ...
        foreach ($categories as $index => $cat) {
            $catId = $cat['categoryId'] ?? ($cat['id'] ?? ($cat['category_id'] ?? $index));
            $catKey = 'cat-' . $catId;
            foreach (($categories_products[$catKey] ?? []) as $product) {
                $product['_category_key'] = $catKey;
                $allProducts[] = $product;
            }
        }
        if (empty($allProducts)): ?>
          <p id="no-products-message" style="color: var(--text-muted); grid-column: 1 / -1; text-align: center; padding: 20px;">No products available right now.</p>
        <?php else:
          foreach ($allProducts as $p):
            $productCategoryKey = $p['_category_key'];
...
<div id="paymongo-info-modal" class="payment-modal-overlay" style="display: none;">
  ...
  <script>
    ...
    function selectCategory(category, card) {
      selectedCategory = category;
      document.querySelectorAll('.lp-cat-card').forEach(item => item.classList.remove('active'));
    ...
    function filterProducts() {
      ...
      const cards = document.querySelectorAll('.inline-product-card');
      let visibleCount = 0;
      cards.forEach(card => {
        const matchesCategory = selectedCategory === 'all' || card.dataset.category === selectedCategory;
        const matchesSearch = card.dataset.productName.includes(search);
        const visible = matchesCategory && matchesSearch;
        card.style.display = visible ? 'flex' : 'none';
        if (visible) visibleCount++;
      });
      document.getElementById('no-search-results').style.display = visibleCount ? 'none' : 'block';
    }
    ...
    window.onload = function() {
      ...
      // If URL contains ?category=ID, select that category in the inline catalog.
      try {
        const params = new URLSearchParams(window.location.search);
        const catId = params.get('category');
        if (catId) {
          const categoryCard = document.querySelector('[data-category-filter="cat-' + catId + '"]');
          if (categoryCard) {
            selectCategory('cat-' + catId, categoryCard);
